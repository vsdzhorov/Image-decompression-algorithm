...
void bmp_rld_assembly(const uint8_t *rle_data, size_t len, size_t width, size_t height, uint8_t *img);
void bmp_rld_assembly_opt(const uint8_t *rle_data, size_t len, size_t width, size_t height, uint8_t *img);
    ...
    if (printRuntime){ // Print runtime and average runtime for the amount of iterations the user has given.
        ...
        case 1:;
            ...
            for (int i = 0; i < iterations; i++){
                bmp_rld_assembly_opt(data, imgSize, width, height, img);
            }
            struct timespec end; 
            clock_gettime(CLOCK_MONOTONIC, &end);
            double time = end.tv_sec - start.tv_sec + 1e-9 * (end.tv_nsec - start.tv_nsec);
            double avg = time/iterations;
            printf("Implementation: Optimized Assembly\n");
            printf("Iterations: %d\n", iterations);
            printf("Runtime: %lf\n", time);
        ...
        case 3:;
            ...
            for (int i = 0; i < iterations; i++){
            bmp_rld_assembly(data, imgSize, width, height, img);
            }
            struct timespec end3; 
            clock_gettime(CLOCK_MONOTONIC, &end3);
            double time3 = end3.tv_sec - start3.tv_sec + 1e-9 * (end3.tv_nsec - start3.tv_nsec);
            double avg3 = time3/iterations;
            printf("Implementation: Basic Assembly\n");
            printf("Iterations: %d\n", iterations);
            printf("Runtime: %lf\n", time3);
            printf("Average runtime: %lf\n", avg3);
            break;
